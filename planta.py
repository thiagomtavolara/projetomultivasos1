...
from IPython.display import display, clear_output #utilizado para atualização dos plotes
#import mtxlab
import warnings
...
tamanhomensagemsplit=0#Olha quantas leituras já foram feitas e na hora de ler a serial do Arduíno faz o código desconsiderar as que já foram lidas.
message=''#Mensagem inicial que corresponde ao que o arduino já mandou para o Jupyter.
...
Peso2='Peso2:'#Quando aparecer isso a mensagem seguinte (após um espaço) é o valor de P2 naquele tempo.
Peso3='Peso3:'#Quando aparecer isso a mensagem seguinte (após um espaço) é o valor de P3 naquele tempo.
Temperatura0='T0:'#Quando aparecer isso a mensagem seguinte (após um espaço) é o valor de T0 naquele tempo.
Temperatura1='T1:'#Quando aparecer isso a mensagem seguinte (após um espaço) é o valor de T1 naquele tempo.
Temperatura2='T2:'#Quando aparecer isso a mensagem seguinte (após um espaço) é o valor de T2 naquele tempo.
Temperatura3='T3:'#Quando aparecer isso a mensagem seguinte (após um espaço) é o valor de T3 naquele tempo.
...
def leitura():
    ...
    while len(messagepesos.split()) < 9:
        messagem=arduino3.readline(1).decode()#Lê o que o arduino mandou letra por letra.
        messagepesos+=messagem#A cada letra a variável mensagem armazena mais alguma coisa.


    while len(message.split()) < 9:
        messagem=arduino.readline(1).decode()#Lê o que o arduino mandou letra por letra.
        message+=messagem#A cada letra a variável mensagem armazena mais alguma coisa.
    ...
    if(len(message.split())>8):#message.split é a mensagem separada a cada espaço, assim se separa palavra por palavra
        um5=message.split()[0+tamanhomensagemsplit]+" "+message.split()[1+tamanhomensagemsplit]#T0: medição
        t0= message.split()[1+tamanhomensagemsplit]
        print(um5)
        um6=message.split()[2+tamanhomensagemsplit]+" "+message.split()[3+tamanhomensagemsplit]#T1: medição
        t1= message.split()[3+tamanhomensagemsplit]
        print(um6)
        um7=message.split()[4+tamanhomensagemsplit]+" "+message.split()[5+tamanhomensagemsplit]#T2: medição
        t2= message.split()[5+tamanhomensagemsplit]
        print(um7)
        um8=message.split()[6+tamanhomensagemsplit]+" "+message.split()[7+tamanhomensagemsplit]#T3: medição
        t3= message.split()[7+tamanhomensagemsplit]
        print(um8)

        mensagemusada=np.delete(message.split(),8)#mensagem em forma de matriz
